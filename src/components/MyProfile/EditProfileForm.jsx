      ...
      <div className="edit-profile-panel">
        <form onSubmit={handleSubmit}>
          <div className="edit-panel-header">
            <h2>{t("editProfileForm.completeYourProfile")}</h2>
          ...
          <div className="edit-panel-content">
            <div className="edit-page-helper-box">
              <strong>{t("editProfileForm.profileSetup")}</strong>
              <p>{t("editProfileForm.addTheBasicsThatHelpClientsTrustYouYourProfilePhotoName")}</p>
            </div>

            <div className="avatar-upload">
              <label htmlFor="avatar" className="avatar-label">
                {imagePreview ? <img src={imagePreview} alt={t("editProfileForm.profileAvatar")} className="avatar-image" /> : <div className="avatar-placeholder">
              ...
              <input type="file" id="avatar" accept="image/*" onChange={handleImageChange} hidden />
            </div>

            <label htmlFor="name">{t("editProfileForm.fullName")}</label>
            <input type="text" id="name" name="name" value={formData.name} onChange={handleChange} />

            <label htmlFor="bio">{t("editProfileForm.bio")}</label>
            <textarea id="bio" name="bio" value={formData.bio} onChange={handleChange} rows={4} placeholder={t("editProfileForm.tellPeopleAboutYourself")} />

            <label htmlFor="country">{t("editProfileForm.country")}</label>
            <div className="suggestion-input-wrap">
              <input type="text" id="country" name="country" value={formData.country} onChange={handleChange} placeholder={t("editProfileForm.typeYourCountry")} autoComplete="off" />

              {filteredCountryOptions.length > 0 && formData.country !== filteredCountryOptions[0] && <ul className="suggestion-list">
            ...
            <label htmlFor="gender">{t("editProfileForm.gender")}</label>
            <select name="gender" id="gender" value={formData.gender} onChange={handleChange}>
              <option value="">{t("editProfileForm.selectGender")}</option>
              <option value="male">{t("editProfileForm.male")}</option>
              <option value="female">{t("editProfileForm.female")}</option>
            </select>

            <label>{t("editProfileForm.languages")}</label>
            ...
            }} placeholder={t("editProfileForm.typeALanguageAndPressEnter")} />
            ...
            <label>{t("editProfileForm.skills")}</label>
            <div className="tags-container">
          ...
          }} placeholder={t("editProfileForm.addASkillEGReact")} />
            <div className="role-section">
              <h3>{t("editProfileForm.profileType")}</h3>
              ...
              <label className="role-card">
                ...
                <strong>{t("editProfileForm.iAmAFreelancerSeller")}</strong>
              </label>
            </div>

            <label>{t("editProfileForm.username")}</label>
            <input type="text" value={profile.username} disabled />
          </div>

          <div className="edit-panel-footer">
            <button type="button" className="cancel-btn" onClick={onClose}>{t("editProfileForm.cancel")}</button>

            <button type="submit" className="save-btn">{t("editProfileForm.save")}</button>
          </div>
        </form>
      </div>
    </>;
