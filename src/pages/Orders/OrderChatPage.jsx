...
function OrderChatPage() {
  ...
  const otherUser = useMemo(() => {
    ...
    return buyerId?.toString() === currentUserId?.toString()
      ? order.seller
      : order.buyer;
  }, [currentUserId, order]);
  ...
  useEffect(() => {
    ...
    async function fetchConversation() {
      try {
        ...
        const normalizedMessages = (msgRes.data.messages || []).map(
          (msg, index, arr) => ({
            ...msg,
            status:
              msg.sender?._id?.toString() === myUserId?.toString() &&
              index === arr.length - 1
                ? "seen"
                : msg.sender?._id?.toString() === myUserId?.toString()
                  ? "sent"
  ...
  async function handleSendMessage(event) {
    ...
    setTimeout(() => {
      setMessages((prev) =>
        prev.map((msg) =>
          msg._id === localMessage._id ? { ...msg, status: "seen" } : msg,
        ),
      );
    }, 1200);
  }
  ...
  if (loading) return <div className="workspace-loading">Loading chat...</div>;
      ...
      <div className="workspace-shell chat-page-shell">
        <header className="workspace-header chat-header">
          <div className="chat-user-header">
            ...
            <div>
              <span className="section-label">{t("chat")}</span>
              <h1>
                {otherUser?.username || order?.service?.title || "Order Chat"}
              </h1>
            </div>
          </div>
